...
type PopulatedAdmin = Pick<Profile, 'full_name' | 'email'>;

const adminName = (adminId: AuditLog['admin_id']) =>
  typeof adminId === 'object' && adminId !== null
    ? ((adminId as unknown as PopulatedAdmin).full_name ?? 'Unknown admin')
    : 'Admin';

export default function AuditLogs() {
  const { data, isLoading: loading, error } = useGetAuditLogsQuery(100);
            ...
            {loading ? (
              ...
              logs.map((log) => (
                <tr key={log.id} className="border-t">
                  <td className="px-6 py-4 text-sm">{adminName(log.admin_id)}</td>
