...
### Admin
- Console shell with dashboard, user/company/job management, reports, audit logs, and security settings screens. **All are UI scaffolds rendering empty state** — no admin endpoints are called.
...
2. **Token key mismatch breaks auth on the central client.** `auth.service.ts` stores the JWT under `token`, but `services/api.ts` reads `auth_token`. Requests made through the shared `ApiClient` send no `Authorization` header. The codebase also carries two parallel HTTP layers (the `ApiClient` class vs. per-service `fetch` helpers) that must be consolidated.
...
6. `src/App.tsx` defines a second, competing router that is unused (`main.tsx` uses `routes/AppRoutes.tsx`). Home.tsx contains ~250 lines of a fully commented-out older revision.
...
8. `main.tsx` has the `index.css` import commented out while `index.html` loads Tailwind via CDN script — fine for prototypes, wrong for production (FOUC, no tree-shaken build).
