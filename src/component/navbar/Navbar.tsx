    ...
    >
      <nav className="mx-auto flex h-20 w-full max-w-7xl flex-row items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden flex-row items-center gap-1 md:flex">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className={`whitespace-nowrap rounded-lg px-4 py-2 text-sm font-semibold transition-all duration-200 ${isActiveLink(link.to)
          ...
          {user ? (
            ...
            <>
              ...
              <Link
                ...
                text-sm font-semibold text-slate-600 shadow-sm shadow-primary-600/30 transition hover:bg-primary-700"
              >
                Get Started
              </Link>
            </>
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center justify-center rounded-lg p-2 text-slate-700 transition hover:bg-slate-100 hover:text-slate-900 md:hidden"
            ...
            <div className="border-t border-slate-200 pt-3">
              {user ? (
                <Link
                  ...
                  className="block rounded-lg px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 hover:text-slate-900"
                >
                  Dashboard
                </Link>
              ) : (
                <div className="grid grid-cols-2 gap-2 pt-1">
                  ...
                  <Link
                    to="/auth/register"
                    className="rounded-lg bg-primary-600 px-4 py-2.5 text-center text-sm font-semibold text-white shadow-sm shadow-primary-600/30 transition hover:bg-primary-700"
                  >
                    Get Started
                  </Link>
                </div>
