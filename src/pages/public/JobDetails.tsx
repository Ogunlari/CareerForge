...
import { fetchJobById, saveJob, unsaveJob, isJobSaved } from '@/services/jobs.service';
import { createApplication, checkExistingApplication } from '@/services/applications.service';
import { useAuth } from '@/context/AuthContext';
import type { JobDetails as Job, StudentProfile } from '@/types';
...
export default function JobDetails() {
    ...
    (async () => {
      ...
      try {
        ...
        if (user?.role === 'student') {
          const [isSaved, existing] = await Promise.all([
            isJobSaved(user.id, id),
            checkExistingApplication(user.id, id),
          ]);
          if (!active) return;
          setSaved(isSaved);
  ...
  const handleApply = async () => {
    ...
    const { error } = await createApplication(user.id, id!, coverLetter, student.resume_url || '');
    if (error) {
      setError(error);
    } else {
      setApplied(true);
  ...
  if (!job) return (
    <div className="pt-16 min-h-screen flex items-center justify-center">
      <div className="text-center">
        <p className="text-slate-400 text-lg">{error || 'Job not found.'}</p>
