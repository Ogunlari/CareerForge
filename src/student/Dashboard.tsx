...
import { fetchSavedJobs, fetchRecommendedJobs } from '@/services/jobs.service';
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [appsResponse, savedResponse, recommendationsResponse] = await Promise.all([
        fetchStudentApplications(user.id),
        fetchSavedJobs(user.id),
        fetchRecommendedJobs(user.id, 3),
      ]);
      setApplications(appsResponse.data ?? []);
      setSavedJobs((savedResponse.data ?? []).map((savedJob) => savedJob.job));
      setRecommended(recommendationsResponse.data ?? []);
      setLoading(false);
    })();
  }, [user]);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/student/recommended" className="text-sm text-primary-600 hover:text-primary-700 font-semibold flex items-center gap-1">
                View all <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
